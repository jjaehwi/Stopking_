...
public class Smoke_Alert_Scanner {
    private Context context;
    private String text;
    private FirebaseFirestore db = FirebaseFirestore.getInstance();
    private String getEmail;

    public Smoke_Alert_Scanner(Smoke_Statistics context, String text, String getEmail) {
        this.context = context;
        this.text = text;
        this.getEmail = getEmail;
    }

    private Smoke_Alert_Scanner.ModifyReturnListener modifyReturnListener;

    public interface ModifyReturnListener{
        void afterModify(String content);
    }
    public void setModifyReturnListener(Smoke_Alert_Scanner.ModifyReturnListener modifyReturnListener){
    ...
    public void callFunction() {
        ...
        conformBtn.setOnClickListener(new View.OnClickListener() { // 수정 버튼 눌렀을 때
            ...
            public void onClick(View view) {
                ...
                dlg.onBackPressed();
                // db 지정 필드에 값 업데이트하기!
                docRef.update("smoke_bank",scaneET.getText().toString());
            }
        });
